...
function appendPnlLogLine(line: string): void {
    ...
    try {
        ...
        logger.warn(`Failed to append pnl.log: ${e instanceof Error ? e.message : String(e)}`);
    }
}
...
function ensurePnlLogExists(): void {
    ...
    try {
        ...
        logger.warn(`Failed to ensure pnl.log exists: ${e instanceof Error ? e.message : String(e)}`);
    }
}
...
function loadCopytradeStateFile(): CopytradeStateFile {
    ...
    try {
        ...
        logger.warn(`Failed to read copytrade-state.json for pnl: ${e instanceof Error ? e.message : String(e)}`);
        return {};
    }
}
...
function pruneCopytradeStateKeepNewest(keep: number): void {
    ...
    try {
        ...
        logger.warn(`Failed to read copytrade-state.json for pruning: ${e instanceof Error ? e.message : String(e)}`);
        return;
    }
    ...
    try {
        ...
        logger.warn(`Failed to write pruned copytrade-state.json: ${e instanceof Error ? e.message : String(e)}`);
    }
}
        ...
        try {
            ...
            if (!dryRun) {
                ...
                try {
                    ...
                    logger.warn(`Failed to get wallet address for balance logging: ${e instanceof Error ? e.message : String(e)}`);
                }
